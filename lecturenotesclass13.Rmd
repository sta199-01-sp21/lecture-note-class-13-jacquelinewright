...
I wouldnt be surprised if there was only 3 people that were
left handed but would be very surprised if there were 40 because 
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-jacquelinewright/manhattan.csv")
```
...
What does this mean?
this tells us how confident we are that our interval will contain the true parameter
...
What does this mean?
we are 95% confident that the true proportion of mice that died while not on treatment is between 69 and 79.3 %
...
Consider the mice radiation results. Suppose we want to compute a 90% 
confidence interval for the proportion of mice that lived while on the 
treatment, regardless of the dose.

```{r practice_2_bootstrap}
boot_prop_practice <- mice %>% 
  filter(treat == 1) %>% 
  mutate(died = if_else(died == 1, "yes", "no")) %>% 
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop_practice, level = .90)
ci_mice_lived
```
...
From the results we can be 90% sure that the proportion of the mice that lived 
while on the treatment is between 55 and 65%.
